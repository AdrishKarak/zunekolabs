...
/* ─── Visual card art SVGs — light bg versions ─── */
function CardVisual({ type, color }: { type: string; color: string }) {
  const c = color;
  /* Shared palette for light bg */
  const BG       = '#f4faf7';          /* card visual area bg */
  const GRID     = 'rgba(10,60,35,0.07)';
  const STROKE   = 'rgba(10,60,35,0.18)';
  const FILL_LO  = 'rgba(10,60,35,0.06)';
  const FILL_MID = 'rgba(10,60,35,0.13)';
  const FILL_HI  = 'rgba(10,60,35,0.22)';
  const GOLD     = '#b8860b';
  ...
  const visuals: Record<string, JSX.Element> = {
    retail: (
      <svg viewBox="0 0 480 200" xmlns="http://www.w3.org/2000/svg" style={{ width: '100%', height: '100%' }}>
        ...
        {[40,120,200,280,360,440].map((x, i) => (
          <rect key={i} x={x} y="30" width="60" height="140" rx="4" fill={FILL_LO} stroke={STROKE} strokeWidth="1"/>
        ))}
        ...
        <rect x="14"  y="155" width="80" height="22" rx="4" fill={FILL_MID} stroke={STROKE} strokeWidth="1"/>
    ...
    erp: (
      <svg viewBox="0 0 480 200" xmlns="http://www.w3.org/2000/svg" style={{ width: '100%', height: '100%' }}>
        ...
        {([[240,100,'HQ'],[100,50,'Plant A'],[380,50,'Plant B'],[80,150,'Finance'],[400,150,'HR'],[240,180,'Supply']] as [number,number,string][]).map(([x, y, label], i) => (
          <g key={i}>
            <circle cx={x} cy={y} r={i===0?22:14} fill={i===0 ? FILL_HI : FILL_LO} stroke={c} strokeWidth={i===0?1.8:1.2} opacity="0.9"/>
        ...
        {[0,1,2].map(i => (
          <circle key={i} cx={240 - 70*Math.cos(i*2.1)} cy={100 - 50*Math.sin(i*2.1)} r="3" fill={c} opacity="0.6"/>
        ))}
        <rect x="14" y="10" width="140" height="28" rx="4" fill={FILL_LO} stroke={STROKE} strokeWidth="1"/>
        <text x="22" y="22" fill={c} fontSize="7" fontFamily="monospace" opacity="0.8">MIGRATION PROGRESS</text>
        <rect x="22" y="26" width="110" height="5" rx="2" fill={GRID}/>
        ...
        <rect x="326" y="10" width="140" height="28" rx="4" fill={FILL_LO} stroke={STROKE} strokeWidth="1"/>
      ...
      <svg viewBox="0 0 480 200" xmlns="http://www.w3.org/2000/svg" style={{ width: '100%', height: '100%' }}>
        ...
        {[0,1,2].map(i => (
          <g key={i} transform={`translate(${30 + i*60}, 30)`}>
            <rect width="50" height="65" rx="3" fill={FILL_LO} stroke={STROKE} strokeWidth="1"/>
            <rect x="6" y="10" width="38" height="3" rx="1" fill={FILL_HI}/>
            <rect x="6" y="17" width="28" height="2" rx="1" fill={FILL_MID}/>
            <rect x="6" y="23" width="32" height="2" rx="1" fill={FILL_MID}/>
            <rect x="6" y="29" width="20" height="2" rx="1" fill={FILL_MID}/>
          </g>
        ))}
        ...
        <rect x="245" y="50" width="90" height="60" rx="6" fill={FILL_MID} stroke={c} strokeWidth="1.5" opacity="0.85"/>
        ...
        <rect x="365" y="30" width="105" height="100" rx="4" fill={FILL_LO} stroke={STROKE} strokeWidth="1"/>
        ...
        <rect x="346" y="155" width="120" height="22" rx="4" fill={FILL_LO} stroke={STROKE} strokeWidth="1"/>
    ...
    qc: (
      <svg viewBox="0 0 480 200" xmlns="http://www.w3.org/2000/svg" style={{ width: '100%', height: '100%' }}>
        <rect width="480" height="200" fill={BG}/>
        <rect x="0" y="120" width="480" height="20" rx="2" fill={FILL_LO} stroke={STROKE} strokeWidth="1"/>
        {[30,90,150,210,270,330,390,450].map((x,i) => (
          <circle key={i} cx={x} cy="130" r="6" fill={FILL_LO} stroke={STROKE} strokeWidth="1"/>
        ))}
        {[60,180,300,420].map((x,i) => (
          <g key={i}>
            <rect x={x-20} y="98" width="40" height="24" rx="2"
              fill={i===2 ? 'rgba(200,40,40,0.08)' : FILL_LO}
              stroke={i===2 ? 'rgba(200,40,40,0.35)' : STROKE}
              strokeWidth="1"/>
            {i===2 && <text x={x} y="113" textAnchor="middle" fill="#c02828" fontSize="6" fontFamily="monospace">DEFECT</text>}
          </g>
        ))}
        <rect x="175" y="20" width="130" height="70" rx="6" fill={FILL_LO} stroke={c} strokeWidth="1.5" opacity="0.8"/>
        <circle cx="240" cy="50" r="18" fill={FILL_LO} stroke={c} strokeWidth="1" opacity="0.7"/>
        <circle cx="240" cy="50" r="8"  fill={FILL_MID} stroke={c} strokeWidth="1"/>
        <text x="240" y="82" textAnchor="middle" fill={c} fontSize="7" fontFamily="monospace" opacity="0.8">CV CAMERA</text>
        <path d="M240 90 L210 122 L270 122 Z" fill={FILL_LO} stroke={STROKE} strokeWidth="1" strokeDasharray="3 4"/>
        ...
        <rect x="14"  y="155" width="110" height="22" rx="4" fill={FILL_LO} stroke={STROKE} strokeWidth="1"/>
    ...
    hr: (
      <svg viewBox="0 0 480 200" xmlns="http://www.w3.org/2000/svg" style={{ width: '100%', height: '100%' }}>
        ...
        {(['Apply','Screen','Interview','Offer','Onboard'] as string[]).map((stage, i) => (
          <g key={i}>
            <rect x={20+i*90} y="60" width="75" height="50" rx="6"
              fill={`rgba(10,60,35,${0.06+i*0.03})`}
              stroke={`rgba(10,60,35,${0.14+i*0.06})`}
              strokeWidth="1"/>
        ...
        <rect x="190" y="28" width="100" height="20" rx="10" fill={FILL_MID} stroke={c} strokeWidth="1" opacity="0.8"/>
        <text x="240" y="42" textAnchor="middle" fill={c} fontSize="8" fontFamily="monospace">⚡ AI SCREENED</text>
        <rect x="14"  y="155" width="130" height="22" rx="4" fill={FILL_LO} stroke={STROKE} strokeWidth="1"/>
        <text x="22"  y="170" fill={c} fontSize="8" fontFamily="monospace">TIME-TO-HIRE: -68%</text>
        <rect x="336" y="155" width="130" height="22" rx="4" fill={FILL_LO} stroke={STROKE} strokeWidth="1"/>
    ...
    supply: (
      <svg viewBox="0 0 480 200" xmlns="http://www.w3.org/2000/svg" style={{ width: '100%', height: '100%' }}>
        ...
        <rect x="280" y="15" width="10" height="3" fill="rgba(10,60,35,0.18)"/>
        <text x="295" y="20" fill="#3d7a55" fontSize="7" fontFamily="monospace">ACTUAL</text>
        ...
        <rect x="350" y="35" width="115" height="40" rx="5" fill={FILL_LO} stroke={STROKE} strokeWidth="1"/>
        ...
        <rect x="14" y="12" width="120" height="22" rx="4" fill={FILL_LO} stroke={STROKE} strokeWidth="1"/>
    ...
    >
      <div style={{
        ...
        <div style={{
          height: '220px',
          overflow: 'hidden',
          borderBottom: '1px solid rgba(10,60,35,0.1)',
          position: 'relative',
          background: '#f4faf7',
        }}>
          ...
          <div style={{
            ...
            color: '#2d6b45',
            border: '1px solid rgba(10,60,35,0.2)',
        ...
        <div style={{ padding: '26px 28px 28px', background: '#ffffff' }}>
          ...
          <p style={{
            ...
            color: '#3d6b50', lineHeight: 1.7, marginBottom: '20px',
          }}>
            {project.desc}
          </p>
          ...
          <div style={{
            ...
            paddingTop: '16px', borderTop: '1px solid rgba(10,60,35,0.1)',
          }}>
            <span style={{
...
export function ShowCaseCards() {
  ...
  const navBtnStyle: React.CSSProperties = {
    ...
    cursor: 'pointer',
    boxShadow: '0 2px 10px rgba(10,60,35,0.1)',
    transition: 'background 0.22s, border-color 0.22s, box-shadow 0.22s, transform 0.18s',
  };

  return (
    <section id="works" style={{ background: '#ffffff', padding: '120px 0', overflow: 'hidden' }}>
      <div style={{ maxWidth: '1280px', margin: '0 auto', padding: '0 32px' }}>
        ...
        >
          ...
          <h2 style={{
            ...
            fontSize: 'clamp(40px, 5vw, 72px)', color: '#0d3d22',
            lineHeight: 1.05, marginBottom: '16px',
          }}>
            What We've Built
          </h2>
          <p style={{ fontFamily: 'var(--font-body)', fontSize: '18px', color: '#3d6b50' }}>
        ...
        <div style={{
          ...
          justifyContent: 'center', height: '520px',
        }}>
          ...
          <button
            onClick={() => navigate(-1)}
            style={{ ...navBtnStyle, position: 'absolute', left: '0px', zIndex: 50 }}
            onMouseEnter={e => {
              e.currentTarget.style.background = '#f0f9f4';
              e.currentTarget.style.borderColor = 'rgba(10,60,35,0.4)';
              e.currentTarget.style.boxShadow = '0 4px 16px rgba(10,60,35,0.15)';
              e.currentTarget.style.transform = 'scale(1.08)';
            }}
            onMouseLeave={e => {
              e.currentTarget.style.background = '#ffffff';
              e.currentTarget.style.borderColor = 'rgba(10,60,35,0.2)';
              e.currentTarget.style.boxShadow = '0 2px 10px rgba(10,60,35,0.1)';
          ...
          <button
            onClick={() => navigate(1)}
            style={{ ...navBtnStyle, position: 'absolute', right: '0px', zIndex: 50 }}
            onMouseEnter={e => {
              e.currentTarget.style.background = '#f0f9f4';
              e.currentTarget.style.borderColor = 'rgba(10,60,35,0.4)';
              e.currentTarget.style.boxShadow = '0 4px 16px rgba(10,60,35,0.15)';
          ...
          {PROJECTS.map((_, i) => (
            <button
              ...
              style={{
                ...
                boxShadow: i === active ? '0 2px 8px rgba(10,60,35,0.25)' : 'none',
              }}
            />
          ))}
        </div>
