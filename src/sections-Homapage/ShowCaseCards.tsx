...
function CardVisual({ type, color }: { type: string; color: string }) {
  ...
  const visuals: Record<string, JSX.Element> = {
    retail: (
      <svg viewBox="0 0 480 200" xmlns="http://www.w3.org/2000/svg" style={{ width: '100%', height: '100%' }}>
        ...
        {[40,120,200,280,360,440].map((x, i) => (
          <rect key={i} x={x} y="30" width="60" height="140" rx="4" fill="rgba(10,66,37,0.04)" stroke="rgba(10,66,37,0.12)" strokeWidth="1"/>
        ))}
        {[40,120,200,280,360].map((x, i) => (
          <g key={i}>
            <rect x={x+8} y="50" width="14" height="30" rx="2" fill={`rgba(10,66,37,${0.15 + i*0.05})`}/>
            <rect x={x+26} y="45" width="14" height="35" rx="2" fill={`rgba(10,66,37,${0.1 + i*0.04})`}/>
            <rect x={x+44} y="55" width="10" height="25" rx="2" fill={`rgba(10,66,37,${0.08 + i*0.03})`}/>
        ...
        <rect x="14" y="155" width="80" height="22" rx="4" fill="rgba(10,66,37,0.1)" stroke="rgba(10,66,37,0.2)" strokeWidth="1"/>
    ...
    erp: (
      <svg viewBox="0 0 480 200" xmlns="http://www.w3.org/2000/svg" style={{ width: '100%', height: '100%' }}>
        ...
        {([[240,100,'HQ'],[100,50,'Plant A'],[380,50,'Plant B'],[80,150,'Finance'],[400,150,'HR'],[240,180,'Supply']] as [number,number,string][]).map(([x, y, label], i) => (
          <g key={i}>
            <circle cx={x} cy={y} r={i===0?22:14} fill={`rgba(10,66,37,${i===0?0.2:0.08})`} stroke={c} strokeWidth={i===0?1.5:1} opacity="0.9"/>
        ...
        <rect x="14" y="10" width="140" height="28" rx="4" fill="rgba(10,66,37,0.06)" stroke="rgba(10,66,37,0.15)" strokeWidth="1"/>
        <text x="22" y="22" fill={c} fontSize="7" fontFamily="monospace" opacity="0.8">MIGRATION PROGRESS</text>
        <rect x="22" y="26" width="110" height="5" rx="2" fill="rgba(10,66,37,0.1)"/>
        ...
        <rect x="326" y="10" width="140" height="28" rx="4" fill="rgba(10,66,37,0.06)" stroke="rgba(10,66,37,0.15)" strokeWidth="1"/>
      ...
      <svg viewBox="0 0 480 200" xmlns="http://www.w3.org/2000/svg" style={{ width: '100%', height: '100%' }}>
        ...
        {[0,1,2].map(i => (
          <g key={i} transform={`translate(${30 + i*60}, 30)`}>
            <rect width="50" height="65" rx="3" fill={`rgba(10,66,37,${0.06+i*0.02})`} stroke="rgba(10,66,37,0.12)" strokeWidth="1"/>
            <rect x="6" y="10" width="38" height="3" rx="1" fill="rgba(10,66,37,0.2)"/>
            <rect x="6" y="17" width="28" height="2" rx="1" fill="rgba(10,66,37,0.12)"/>
            <rect x="6" y="23" width="32" height="2" rx="1" fill="rgba(10,66,37,0.12)"/>
            <rect x="6" y="29" width="20" height="2" rx="1" fill="rgba(10,66,37,0.12)"/>
          </g>
        ))}
        ...
        <rect x="245" y="50" width="90" height="60" rx="6" fill="rgba(10,66,37,0.12)" stroke={c} strokeWidth="1.5" opacity="0.85"/>
        ...
        <rect x="365" y="30" width="105" height="100" rx="4" fill="rgba(10,66,37,0.06)" stroke="rgba(10,66,37,0.18)" strokeWidth="1"/>
        ...
        <rect x="346" y="155" width="120" height="22" rx="4" fill="rgba(10,66,37,0.08)" stroke="rgba(10,66,37,0.2)" strokeWidth="1"/>
    ...
    qc: (
      <svg viewBox="0 0 480 200" xmlns="http://www.w3.org/2000/svg" style={{ width: '100%', height: '100%' }}>
        <rect width="480" height="200" fill="#080808"/>
        <rect x="0" y="120" width="480" height="20" rx="2" fill="rgba(10,66,37,0.05)" stroke="rgba(10,66,37,0.1)" strokeWidth="1"/>
        {[30,90,150,210,270,330,390,450].map((x,i) => (
          <circle key={i} cx={x} cy="130" r="6" fill="rgba(10,66,37,0.08)" stroke="rgba(10,66,37,0.2)" strokeWidth="1"/>
        ))}
        {[60,180,300,420].map((x,i) => (
          <g key={i}>
            <rect x={x-20} y="98" width="40" height="24" rx="2" fill={i===2?'rgba(255,60,60,0.12)':'rgba(10,66,37,0.08)'} stroke={i===2?'rgba(255,80,80,0.4)':'rgba(10,66,37,0.2)'} strokeWidth="1"/>
            {i===2 && <text x={x} y="113" textAnchor="middle" fill="#ff5050" fontSize="6" fontFamily="monospace">DEFECT</text>}
          </g>
        ))}
        <rect x="175" y="20" width="130" height="70" rx="6" fill="rgba(10,66,37,0.08)" stroke={c} strokeWidth="1.5" opacity="0.8"/>
        <circle cx="240" cy="50" r="18" fill="rgba(10,66,37,0.1)" stroke={c} strokeWidth="1" opacity="0.7"/>
        <circle cx="240" cy="50" r="8" fill="rgba(10,66,37,0.2)" stroke={c} strokeWidth="1"/>
        <text x="240" y="82" textAnchor="middle" fill={c} fontSize="7" fontFamily="monospace" opacity="0.8">CV CAMERA</text>
        <path d="M240 90 L210 122 L270 122 Z" fill="rgba(10,66,37,0.04)" stroke="rgba(10,66,37,0.15)" strokeWidth="1" strokeDasharray="3 4"/>
        ...
        <rect x="14" y="155" width="110" height="22" rx="4" fill="rgba(10,66,37,0.08)" stroke="rgba(10,66,37,0.2)" strokeWidth="1"/>
    ...
    hr: (
      <svg viewBox="0 0 480 200" xmlns="http://www.w3.org/2000/svg" style={{ width: '100%', height: '100%' }}>
        ...
        {(['Apply','Screen','Interview','Offer','Onboard'] as string[]).map((stage, i) => (
          <g key={i}>
            <rect x={20+i*90} y="60" width="75" height="50" rx="6" fill={`rgba(10,66,37,${0.06+i*0.025})`} stroke={`rgba(10,66,37,${0.1+i*0.06})`} strokeWidth="1"/>
            <text x={57+i*90} y="82" textAnchor="middle" fill={c} fontSize="8" fontFamily="monospace" opacity="0.85">{stage}</text>
            <rect x={45+i*90} y="92" width="24" height="12" rx="3" fill={`rgba(10,66,37,${0.12+i*0.04})`}/>
        ...
        <rect x="190" y="28" width="100" height="20" rx="10" fill="rgba(10,66,37,0.1)" stroke={c} strokeWidth="1" opacity="0.7"/>
        <text x="240" y="42" textAnchor="middle" fill={c} fontSize="8" fontFamily="monospace">⚡ AI SCREENED</text>
        <rect x="14" y="155" width="130" height="22" rx="4" fill="rgba(10,66,37,0.08)" stroke="rgba(10,66,37,0.2)" strokeWidth="1"/>
        <text x="22" y="170" fill={c} fontSize="8" fontFamily="monospace">TIME-TO-HIRE: -68%</text>
        <rect x="336" y="155" width="130" height="22" rx="4" fill="rgba(10,66,37,0.06)" stroke="rgba(10,66,37,0.15)" strokeWidth="1"/>
    ...
    supply: (
      <svg viewBox="0 0 480 200" xmlns="http://www.w3.org/2000/svg" style={{ width: '100%', height: '100%' }}>
        <rect width="480" height="200" fill="#080808"/>
        <polyline points="30,150 80,130 130,140 180,100 230,120 280,80 330,95 380,60 430,75" fill="none" stroke="rgba(10,66,37,0.15)" strokeWidth="2" strokeDasharray="6 4"/>
        ...
        <line x1="20" y1="30" x2="20" y2="165" stroke="rgba(10,66,37,0.15)" strokeWidth="1"/>
        <line x1="20" y1="165" x2="460" y2="165" stroke="rgba(10,66,37,0.15)" strokeWidth="1"/>
        <rect x="280" y="15" width="10" height="3" fill="rgba(10,66,37,0.15)"/>
        ...
        <rect x="350" y="35" width="115" height="40" rx="5" fill="rgba(10,66,37,0.08)" stroke="rgba(10,66,37,0.2)" strokeWidth="1"/>
        ...
        <rect x="14" y="12" width="120" height="22" rx="4" fill="rgba(10,66,37,0.06)" stroke="rgba(10,66,37,0.15)" strokeWidth="1"/>
...
function CardSlot({
  ...
  return (
    ...
    >
      <div style={{
        borderRadius: '16px',
        border: `1px solid ${isActive ? 'rgba(10,66,37,0.45)' : 'rgba(10,66,37,0.12)'}`,
        background: 'var(--bg-surface)',
        overflow: 'hidden',
        boxShadow: isActive
          ? '0 32px 80px rgba(0,0,0,0.6), 0 0 60px rgba(10,66,37,0.12)'
        ...
        <div style={{
          height: '220px',
          overflow: 'hidden',
          borderBottom: '1px solid rgba(10,66,37,0.08)',
          position: 'relative',
          ...
          <div style={{
            ...
            color: 'var(--text-secondary)', border: '1px solid rgba(10,66,37,0.12)',
        ...
        <div style={{ padding: '26px 28px 28px' }}>
          ...
          <div style={{
            ...
            paddingTop: '16px', borderTop: '1px solid rgba(10,66,37,0.08)',
          }}>
            <span style={{
        ...
        <div style={{
          ...
          <button
            ...
            style={{
              ...
              boxShadow: '0 0 20px rgba(10,66,37,0.15)',
            }}
            onMouseEnter={e => { e.currentTarget.style.background = 'rgba(10,66,37,0.15)'; e.currentTarget.style.transform = 'scale(1.1)'; }}
          ...
          {PROJECTS.map((_, i) => (
            <button
              ...
              style={{
                ...
                background: i === active ? 'var(--accent-primary)' : 'rgba(10,66,37,0.2)',
                border: 'none', cursor: 'pointer', padding: 0,
                transition: 'all 0.4s var(--ease-smooth)',
                boxShadow: i === active ? '0 0 12px rgba(10,66,37,0.4)' : 'none',
              }}
            />
          ))}
        </div>
